...
vector<int> topViewBinaryTree(Node* root){
    ...
    while(!q.empty()){
        ...
        while(nodeAtCurrentLevel--){
            ...
            // If any node is not present at that column, then store the value of that node in the map at that column Index value(m[column] = node->data)
...
int main() {
    ...
    // Top view of the binary tree
    vector<int> result = topViewBinaryTree(root);
