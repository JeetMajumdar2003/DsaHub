...
void bucketsort(float arr[], int n){
    ...
    // Step-2: Inserting elements into the bucket at:(ma'am approach)
    /*
    ** Index calculation:
    ...
    */
    // 1. First calculate the range of elements:
    int range = (*max_element(arr, arr+n) - *min_element(arr, arr+n))/n; // here max_element() and min_element() returns the address of the max and min element respectively. So, we have to dereference(*) them to get the value.
    //  2. Now, calculate the index of each element and insert them into the bucket:
    for (int i = 0; i < n; i++){
        int idx = (arr[i] - *min_element(arr, arr+n))/range;
        // For boundary case:(if the element is the max element) idx will be equal to n. So, we have to put it in the last bucket.
        if(idx == n){
            idx--;
        }
